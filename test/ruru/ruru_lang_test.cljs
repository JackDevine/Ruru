...
(deftest interpret-test
  (testing "Interpret expressions"
    ...
    (is (= (first (ruru/interpret "2{x-y*2}3" env))
           -2))
    (is (= (-> "['a,2]#@'a" (ruru/interpret ruru/default-environment) first)
           2))
    (is (= (-> "[\"a\",2]#@\"a\"" (ruru/interpret ruru/default-environment) first)
           2))
    (is (= (-> "[[1],[2]]Reduce[{[(x@1)+(y@1)]},[3]]" (ruru/interpret ruru/default-environment) first)
           {'array_dims [1 1] 'value [6]}))
    (is (= (-> "[1,2]Reduce[{x+y},3]" (ruru/interpret ruru/default-environment) first)
           6))
    (is (= (first (ruru/interpret "sq:={x Square}\n2 Sq" env))
           4))
...
;;     (is (= (-> (ruru/interpret "1:3Map({x Square})" ruru/default-environment) first)
;;            {'array_dims [3 1] 'value [1 4 9]}))
